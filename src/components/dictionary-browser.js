...
export default function DictionaryBrowser({ searchTerm, letterFilter, colors }) {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && !loadingMore && !searchTerm && !letterFilter) {
          loadMoreWords();
        }
      },
      { threshold: 0.1 }
  ...
  return (
    <div>
      {selectedWord && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setSelectedWord(null)}>
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full max-h-[80vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-4">
              <div>
                <h2 className="text-2xl font-bold text-blue-900">{selectedWord.word}</h2>
            ...
            <div className="space-y-4">
                  ...
                  <div key={index} className="border-b border-gray-100 pb-4">
                    <div className="mb-2">
                      <span className="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded-full">
                        Definition {selectedWord.senses.length > 1 ? index + 1 : ''}
                      </span>
                    </div>
                    
                    {sense.definitions && sense.definitions.map((def, i) => (
                    ...
                    {sense.examples && sense.examples.length > 0 && (
                      <div className="mt-3 pl-4 border-l-2 border-blue-200">
                        <p className="text-sm text-gray-500 mb-1 font-medium">Example:</p>
                    ...
                    {sense.relations && sense.relations.length > 0 && (
                      <div className="mt-3">
                        <h4 className="text-sm font-medium mb-1">Related concepts:</h4>
              ...
              {selectedWord.spellings && selectedWord.spellings.length > 0 && (
                <div className="border-t border-gray-100 pt-4">
                  <h4 className="text-sm font-medium mb-2">Alternative spellings:</h4>
              ...
              {!savedWords.some(saved => saved.word === selectedWord.word) && (
                <button
                  onClick={() => {
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  ...
                  <button 
                    className={`p-2 rounded-full ${isSaved ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-blue-100 text-blue-600 hover:bg-blue-200'}`}
                    onClick={(e) => {
