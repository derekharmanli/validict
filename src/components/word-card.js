...
export default function WordCard({ word, onRemove }) {
  ...
  const { addTag, removeTag, markReviewed } = useWordStore();
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div className="flex justify-between items-center mt-4 text-xs text-muted-foreground">
          <span>Added {formatDistanceToNow(new Date(word.dateAdded))} ago</span>
          {word.lastReviewed && (
            <span>
              Last reviewed {formatDistanceToNow(new Date(word.lastReviewed))}{" "}
              ago
            </span>
          )}
        </div>
